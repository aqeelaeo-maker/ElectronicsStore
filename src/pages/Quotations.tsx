...
export default function Quotations() {
  ...
  // Generate HTML for Quotation Printing and PDF Download
  const generateQuotationHtml = (quote: Quotation): string => {
  ...
  // Download Quotation as high-quality PDF
  const handleDownloadQuotation = async (quote: Quotation) => {
    try {
      setDownloadingQuoteId(quote.id);
      toast.info(`Preparing PDF for Quotation #${quote.quotationNo}...`);
      const htmlContent = generateQuotationHtml(quote);
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            {editingQuotation && (
              <button
                type="button"
                onClick={() => handleDownloadQuotation(editingQuotation)}
                disabled={downloadingQuoteId === editingQuotation.id}
                className="flex-1 lg:flex-none px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl border border-blue-200 transition-colors shadow-xs text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                title="Download Quotation as PDF"
              >
                {downloadingQuoteId === editingQuotation.id ? (
                    ...
                    <td className="px-3 py-3 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-blue-600 hover:text-blue-800 hover:bg-blue-50 transition-colors cursor-pointer disabled:opacity-50"
